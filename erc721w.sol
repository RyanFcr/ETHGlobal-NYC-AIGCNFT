...
pragma solidity ^0.8.0;
contract AiNft is ERC721Enumerable, IERC721W, Ownable{
    using EnumerableSet for EnumerableSet.AddressSet;

    string _uri;
    EnumerableSet.AddressSet _supportedBcns;
    mapping(address => uint256) _supplyOfBcn;
    mapping(address => uint256) _remaingOfBcn;
    mapping(address => mapping(uint256 => bool)) _isTokenMintByBcn;
    struct Config{
        address fee1;
        address fee2;
        uint128 amount1;
    ...
    constructor(string memory name, string memory symbol, string memory uri, Config memory _config) ERC721(name, symbol){
        _uri = uri;
        config = _config;
    }
    ...
    function mintByBcn(uint256 tokenId, address bcn, uint256 bcnTokenId) external override{
        ...
        _remaingOfBcn[bcn]--;
        _mint(to, tokenId);
        emit MintByBCN(tokenId, to, bcn, bcnTokenId);
    }
    ...
    function mint(address to, uint256 tokenId) external payable{
        ...
        require(msg.value == (c.amount1 + c.amount2), "mint:value error");
        payable(c.fee1).transfer(c.amount1);
        payable(c.fee2).transfer(c.amount2);
        _mint(to, tokenId);
    }
